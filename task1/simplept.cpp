...
//Path-tracing Version 0.5 (Task 1 of Part 2 of Project 2)
...
Vec radiance(const Ray &r, const Sphere &thisSphere, Vec xN, int depth) {
	Vec rad;
	rad = thisSphere.e + reflectedRadiance(r, thisSphere, xN, depth);
	return rad;
}
...
Vec reflectedRadiance(const Ray &r, const Sphere &s, Vec xN, int depth) {		// r has the current  position and the direction of the ray pointing to the previous x poistion
	...
	if (randVal < p) {
		...
		rad = (radiance(outgoing, obj, n, (depth + 1)) * PI).mult(s.brdf.eval(xN, r.d, incDir)) * (1.0 / p);
	}
	else {
		rad = Vec();
	}
