...
//Path-tracing Version 0.5 withj specular BRDF, for Part 2, Task 2 of Project 2
...
Vec radiance(const Ray &r, const Sphere &thisSphere, Vec xN, int depth);
...
Vec radiance(const Ray &r, const Sphere &thisSphere, Vec xN, int depth) {
	Vec rad;
	rad = reflectedRadiance(r, thisSphere, xN, depth);
	return rad;
}
...
Vec reflectedRadiance(const Ray &r, const Sphere &s, Vec xN, int depth) {		// r has the current  position and the direction of the ray pointing to the previous x poistion
	Vec incDir, rad;
	int rrDepth = 5;
	float surviveProbability = 0.9;
	float p, randVal;
	double probDF, t;                                   // t = Distance to intersection
	int id = 0;                                 // id of intersected sphere
	...
	if (randVal < p) {
		...
		rad = rad + radiance(outgoing, obj, n, (depth + 1)).mult(s.brdf.eval(xN, r.d, incDir)) * (1.0 / probDF) * (1.0 / p);
	}
	return rad;
}
